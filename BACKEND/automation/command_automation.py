...
class SmartShellEngine:
    ...
    async def generate_command(self, user_request: str) -> str:
        ...
        try:
            cmd = await ai_engine.chat(messages, model=None, temperature=0.0, max_tokens=512)
        except Exception as e:
            raise Exception(f"No AI provider available for shell command generation: {e}")
    ...
    async def describe_command(self, command: str) -> str:
        ...
        try:
            return await ai_engine.chat(messages, model=None, temperature=0.3, max_tokens=256)
        except Exception:
    ...
    async def generate_code(self, user_request: str, language: str = "python") -> str:
        ...
        try:
            code = await ai_engine.chat(messages, model=None, temperature=0.1, max_tokens=2048)
        except Exception:
...
class SmartShellCommand(BaseModel):
    ...
    @classmethod
    async def execute(cls, description: str, auto_execute: bool = False) -> str:
        engine = SmartShellEngine()
        result = await engine.smart_execute(description, auto_execute=auto_execute)
